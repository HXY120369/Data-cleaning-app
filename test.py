...
class DataAuditApp:
    def __init__(self, master, initial_path=None):
        ...
        self.reselect_button = ttk.Button(file_controls_frame, text="重新选择文件", command=self.load_file)
        ...
        try:
            if filepath.lower().endswith(('.xlsx', '.xls')):
                ...
                try:
                    ...
                    self.df = pd.read_csv(filepath, encoding='gbk')  # 尝试GBK
            else:
                messagebox.showerror("文件类型错误", "不支持的文件类型。请选择 Excel 或 CSV 文件。")
                return
    ...
    def populate_all_tabs(self):
        ...
        overview_content = f"文件名：{file_name}\n"
        overview_content += f"文件路径: {self.filepath}\n"
